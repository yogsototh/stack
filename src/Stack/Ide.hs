...
ide targets useropts = do
             ...
             do cabalfp <- getCabalFileName dir
                ...
                if validWanted && wanted pwd cabalfp pkg
                    then do
                        ...
                        autogenDir <- liftM autogenDir (distDirFromDir dir)
                        paths_foo <-
                            liftM
                                (autogenDir </>)
                                (parseRelFile
                                     ("Paths_" ++ packageNameString name ++ ".hs"))
    ...
    exec
        "ide-backend-client"
        ("empty" : map ("--ghc-option=" ++) (pkgopts <> useropts) <> pkgdbs)
...
exec cmd args input = do
    ...
    liftIO (do S.hPutStrLn stdout (L.toStrict input)
               S.hPutStrLn procin (L.toStrict input))
    _tid <-
        liftIO
            (forkIO
                 (forever
