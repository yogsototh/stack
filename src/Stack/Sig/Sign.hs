...
signTarBytes :: (Applicative m, MonadCatch m, MonadBaseControl IO m, MonadIO m, MonadMask m, MonadLogger m, MonadThrow m)
...
signAll :: (Applicative m, MonadCatch m, MonadBaseControl IO m, MonadIO m, MonadMask m, MonadLogger m, MonadThrow m)
        => String -> String -> m ()
signAll url uname = do
    ...
    forM_
        (filter
             (\name ->
                   show name `elem`
                   map (\(PackageIdentifier n _) -> show n) fromHackage)
             fromIndex)
        (\pkg ->
           -- TODO fetch with stack instead of cabal-install
              do cabalFetch
                     ["--no-dependencies"]
                     pkg
...
signPackage :: (Applicative m, MonadCatch m, MonadBaseControl IO m, MonadIO m, MonadMask m, MonadLogger m, MonadThrow m)
